;
; GPIO_Input_Pull_Up.asm
;
; Created: 10/31/2024
; Author : GM
; Desc   : Toggle LED on and off with each button press, 
;          GPIO Pull Up - Pin high while circuit open
; Chip   : aTmega328
; -----------------------------------------------------------
...
main:
          ...
          sbi       PORTD, PORTD2       ; clear D2 for pull-up


main_loop:                              ; main loop() {

ck_in_D2:                               ; label for button loop
          sbic      PIND, PIND2         ; skip rjmp when pin goes low
          rjmp      ck_in_D2            ; wait for pin to be pulled low
...
ck_in_D2_again:                         ; label for button loop2 - so it stays on line 34 and does go up
          sbic      PIND, PIND2         ; skip rjmp when pin goes low
          rjmp      ck_in_D2_again      ; wait for pin to be pulled low
          
          cbi       PORTB, PORTB2       ; Clear PB2 to turn of LED
          call      delay_1s            ; Call delay for 1 second
